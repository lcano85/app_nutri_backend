...
let PermissionsGuard = class PermissionsGuard {
    ...
    async canActivate(context) {
        ...
        const user = await this.prisma.user.findUnique({
            where: { id: userId },
            select: { role: { select: { active: true, permissions: { select: { permission: { select: { code: true } } } } } } },
        });
        const granted = new Set(user?.role.permissions.map(item => item.permission.code) ?? []);
        if (!user?.role.active || !required.every(code => granted.has(code)))
